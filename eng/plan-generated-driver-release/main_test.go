...
func TestValidateReleaseContractRejectsInvalidStaticLibraryPath(t *testing.T) {
	t.Parallel()
	for _, test := range []struct {
		name   string
...
func TestValidateReleaseContractRejectsLegacySchema(t *testing.T) {
	t.Parallel()
	root := writeContractFixture(t, "0.1.0")
	manifest := readContractProvenance(t, root)
	...
	if err == nil || !strings.Contains(err.Error(), "unsupported provenance schema_version 1") {
		t.Fatalf("validateReleaseContract() error = %v, want schema 1 rejection", err)
	}
}
	...
	for _, test := range []struct {
		...
		},
		{
			...
			func(manifest *provenance) {
				manifest.Targets = append(manifest.Targets, provenanceTarget{
					ModulePath:        "freebsd/amd64",
					StaticLibraryPath: "freebsd/amd64/libazurecosmosdriver.a",
				})
			},
			"extra=[freebsd/amd64]",
		},
